...
/**
 * Plugin Name: Stories
 * Description: Allows users to post temporary stories (photo/video) that disappear after 2 minutes.
 * Version: 1.0
 * Author: Mert Korkmaz
 */
...
function story_upload_form() {
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <p>
            <input type="file" name="story_media" id="story_media" accept="image/*,video/*" required hidden>
            <button type="button" class="custom-upload-btn" onclick="document.getElementById('story_media').click()">Choose a photo or video</button>
        </p>
...
function handle_story_upload() {
    if (isset($_POST['submit_story']) && is_user_logged_in()) {
        ...
        wp_redirect(remove_query_arg('submit_story', wp_get_referer()));
        exit;
    }
}
